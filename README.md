...
An original dataset of NAS and enterprise hard drives: full specs, **CMR vs SMR classification**, and **annualized failure rates** derived from Backblaze Drive Stats. 273 drives.
...
**Price trends:** [`price-trends/`](price-trends/) - two charts of the NAS drive market over time: the monthly median \$/TB (consumer vs enterprise, the storage price crunch), and how often each drive goes on sale and how deep. Computed data plus the charts.

| File | Rows | What it is |
|------|------|------------|
| [`data/drives-2026-09-04.csv`](data/drives-2026-09-04.csv) | 273 | The headline file: one flat row per drive, all specs and failure rate combined. |
| [`data/drives-2026-09-04.json`](data/drives-2026-09-04.json) | 273 | The same rows as JSON (mirror of the live API). |
| [`data/cmr-reference-2026-09-04.csv`](data/cmr-reference-2026-09-04.csv) | 273 | Source of truth for specs and CMR/SMR classification (including idle/seek noise and helium fill), with a per-row `confidence` flag. Our original research. |
| [`data/backblaze-afr-2026-09-04.csv`](data/backblaze-afr-2026-09-04.csv) | 31 | Annualized failure rates over a rolling window through Q2 2026 (full-year 2025 plus the Q1 and Q2 2026 quarters), computed from Backblaze raw Drive Stats. |
| [`data/series-reliability-2026-09-04.csv`](data/series-reliability-2026-09-04.csv) | - | Manufacturer series specs (warranty, rated workload, MTBF) used as context where Backblaze has no data. |
...
## Columns (`drives-2026-09-04.csv` / `drives-2026-09-04.json`)
...
| `afr_pct` | Annualized failure rate over a rolling window through Q2 2026 (2025 + the Q1 and Q2 2026 quarters), derived from Backblaze Drive Stats (null where Backblaze doesn't track the model). |
...
- **Failure rates** are computed from Backblaze's raw quarterly Drive Stats: we aggregate drive-days and failure counts into a single annualized rate over a rolling window through Q2 2026 (full-year 2025 plus the Q1 and Q2 2026 quarters), which keeps per-model samples large, and validate against Backblaze's published fleet figures (~1.41% over that window). The computation and methodology are ours; the underlying counts are Backblaze's (see licence section). Models Backblaze doesn't track are left null rather than guessed.
- **`series-reliability-2026-09-04.csv`** gives manufacturer-rated context (warranty, workload TB/year, MTBF) for lines Backblaze doesn't run.
...
The fields `afr_pct`, `reliability_drive_count`, `reliability_drive_days`, `reliability_failures` (in `drives-2026-09-04.csv` / `drives-2026-09-04.json`) and all of `backblaze-afr-2026-09-04.csv` are derived from Backblaze Drive Stats. The computation is ours; the underlying counts are Backblaze's.
...
- **2026-09-04 - v1.0.4** - Refreshed Backblaze failure rates to a rolling window through Q2 2026 (full-year 2025 plus the Q1 and Q2 2026 quarters, ~1.41% fleet), computed from Backblaze's raw Q2 2026 Drive Stats; 273 drives (167 HDD + 106 SSD), 25 with a failure rate. Added recent PCIe 5.0 and NAS-cache SSDs (Crucial P510, Corsair MP700 Elite, Synology SNV3410/SNV3510); sub-TB drive capacities are now reported correctly (were 0). NAS enclosures are no longer included.
